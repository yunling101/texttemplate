...
func New(template, startTag, endTag string) (t *Template, err error) {
...
func (t *Template) Reset(template, startTag, endTag string) error {
	...
	if len(startTag) == 0 {
		return fmt.Errorf("%s", "startTag cannot be empty")
	}
	if len(endTag) == 0 {
		return fmt.Errorf("%s", "endTag cannot be empty")
	}
...
func stdTagFunc(w io.Writer, tag string, m map[string]interface{}) (int, error) {
	...
	case TagFunc:
		return value(w, tag)
	case int, int64, int32, int8, int16, float64, float32:
